...
<script lang="ts">
    ...
    const transactions = $derived(page.props.transactions as {
        data: Array<{
            id: number;
            type: string;
            amount: number;
    ...
    const filters = $derived(page.props.filters as {
        type?: string;
        category_id?: string;
        payment_method?: string;
        search?: string;
    ...
    function handleSearch() {
        ...
        router.get(`/transactions?${params.toString()}`, {}, { preserveState: true, preserveScroll: true });
    }
...
<div class="flex flex-col gap-6 p-4">
        ...
        <Card>
            <CardContent class="grid gap-4 pt-6 sm:grid-cols-2 lg:grid-cols-4">
                <div>
                    ...
                    >
                        <SelectTrigger>
                            <span class={filters.type ? '' : 'text-muted-foreground'}>
                                {filters.type === 'expense' ? 'مصروف' : filters.type === 'income' ? 'إيراد' : 'الكل'}
                            </span>
                        </SelectTrigger>
                        <SelectContent>
                ...
                <div>
                    ...
                    >
                        <SelectTrigger>
                            <span class={filters.category_id ? '' : 'text-muted-foreground'}>
                                {categories?.find(c => String(c.id) === filters.category_id)?.name || 'الكل'}
                            </span>
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem value="all">الكل</SelectItem>
                ...
                <div>
                    ...
                    >
                        <SelectTrigger>
                            <span class={filters.payment_method ? '' : 'text-muted-foreground'}>
                                {paymentMethods[filters.payment_method as keyof typeof paymentMethods] || 'الكل'}
                            </span>
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem value="all">الكل</SelectItem>
                            {#each Object.entries(paymentMethods) as [key, label]}
    ...
    {#if transactions?.data?.length}
                ...
                <div class="flex items-center gap-4 border-b p-4 last:border-0">
                    ...
                    <div class="flex-1 text-right">
                        ...
                        <p class="text-sm text-muted-foreground">
                            ...
                            · {paymentMethods[tx.payment_method as keyof typeof paymentMethods] || tx.payment_method}
                            · {tx.transaction_date}
                        </p>
                    </div>
        ...
        <p class="text-center text-sm text-muted-foreground">
            عرض {transactions.from} - {transactions.to} من إجمالي {transactions.total} معاملة
        </p>
    {:else}
